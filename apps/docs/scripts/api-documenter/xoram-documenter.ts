...
import type { IMarkdownDocumenterOptions } from '@microsoft/api-documenter/lib/documenters/MarkdownDocumenter.js';
...
import { DocListItem } from './doc-nodes/doc-list-item.ts';
import { DocList } from './doc-nodes/doc-list.ts';
import { XoramMarkdownEmitter } from './xoram-markdown-emitter.ts';

export default class XoramDocumenter {
	...
	public constructor(options: IMarkdownDocumenterOptions) {
		...
		this._markdownEmitter = new XoramMarkdownEmitter(this._apiModel);
		...
		if (apiItem instanceof ApiTypeAlias) {
			const refs: ExcerptToken[] = apiItem.excerptTokens.filter(
				(token) =>
					...
					this._apiModel.resolveDeclarationReference(token.canonicalReference).resolvedApiItem,
			);
			if (refs.length > 0) {
	...
	private _writePackageOrNamespaceTables(output: DocSection, apiContainer: ApiPackage | ApiNamespace): void {
		...
		const abstractClassesTable: DocList = new DocList({
			configuration,
			// headerTitles: [ 'Abstract Class', 'Description' ],
		});

		const classesTable: DocList = new DocList({
			configuration,
			// headerTitles: [ 'Class', 'Description' ],
		});

		const enumerationsTable: DocList = new DocList({
			configuration,
			// headerTitles: [ 'Enumeration', 'Description' ],
		});

		const functionsTable: DocList = new DocList({
			configuration,
			// headerTitles: [ 'Function', 'Description' ],
		});

		const interfacesTable: DocList = new DocList({
			configuration,
			// headerTitles: [ 'Interface', 'Description' ],
		});

		const namespacesTable: DocList = new DocList({
			configuration,
			// headerTitles: [ 'Namespace', 'Description' ],
		});

		const variablesTable: DocList = new DocList({
			configuration,
			// headerTitles: [ 'Variable', 'Description' ],
		});

		const typeAliasesTable: DocList = new DocList({
			configuration,
		...
		for (const apiMember of apiMembers) {
			const listItem = new DocListItem({ configuration }, [
				this._createTitleCell(apiMember),
				this._createDescriptionCell(apiMember),
			]);
			...
			switch (apiMember.kind) {
				case ApiItemKind.Class: {
					if (ApiAbstractMixin.isBaseClassOf(apiMember) && apiMember.isAbstract) {
						abstractClassesTable.addItem(listItem);
					}
					else {
						classesTable.addItem(listItem);
					}
					this._writeApiItemPage(apiMember);
					break;
				}
				...
				case ApiItemKind.Enum: {
					enumerationsTable.addItem(listItem);
					this._writeApiItemPage(apiMember);
					break;
				}

				case ApiItemKind.Interface: {
					interfacesTable.addItem(listItem);
					this._writeApiItemPage(apiMember);
					break;
				}

				case ApiItemKind.Namespace: {
					namespacesTable.addItem(listItem);
					this._writeApiItemPage(apiMember);
					break;
				}

				case ApiItemKind.Function: {
					functionsTable.addItem(listItem);
					this._writeApiItemPage(apiMember);
					break;
				}

				case ApiItemKind.TypeAlias: {
					typeAliasesTable.addItem(listItem);
					this._writeApiItemPage(apiMember);
					break;
				}

				case ApiItemKind.Variable: {
					variablesTable.addItem(listItem);
					this._writeApiItemPage(apiMember);
					break;
				}
			}
	...
	private _appendExcerptTokenWithHyperlinks(docNodeContainer: DocNodeContainer, token: ExcerptToken): void {
		...
		const unwrappedTokenText: string = token.text.replaceAll(/[\r\n]+/g, ' ');
