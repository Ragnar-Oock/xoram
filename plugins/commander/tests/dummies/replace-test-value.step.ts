...
export class ReplaceTestValueStep implements Step {
	...
	public apply(state: State): Result<State> {
		...
		if (typeof realm?.value === 'string') {
			const from = this.from < 0 ? state.realms[this.realm].value.length + this.from + 1 : this.from;
			const to = this.to < 0 ? state.realms[this.realm].value.length + this.to + 1 : this.to;
	...
	public reverse(state: State): Step {
		...
		return new ReplaceTestValueStep(
			...
			(state.realms[this.realm] as { value?: string }).value.slice(from, to),
		);
	}
